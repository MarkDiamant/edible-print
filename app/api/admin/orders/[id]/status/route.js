...
export async function POST(req,{params}){
  ...
  try{
    await sendOrderEmail(id,change.emailType);
    if(action==='fulfilled'){
    ...
    await db.from('order_events').insert({order_id:id,event_type:'email_failed',actor:'system',details:{type:change.emailType,message:String(emailError?.message||emailError)}});
  }
