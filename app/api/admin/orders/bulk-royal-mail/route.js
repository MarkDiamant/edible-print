...
export async function POST(req){
  ...
  for(const order of orders||[]){
    ...
    const {data:existing}=await db.from('order_events').select('details,created_at').eq('order_id',order.id).eq('event_type','royal_mail_order_created').order('created_at',{ascending:false}).limit(1);
    if(existing?.length){
      const stillThere=await remoteOrderExists(apiKey,existing[0].details||{});
      if(stillThere){skipped.push(order.id);continue}
      await db.from('order_events').insert({order_id:order.id,event_type:'royal_mail_order_deleted',actor:'sync',details:{...(existing[0].details||{}),message:'Order no longer exists in Click & Drop'}});
    }
  ...
  for(const p of prepared){
    ...
    if(created){
      ...
      await db.from('order_events').insert({order_id:p.order.id,event_type:'royal_mail_order_failed',actor:'admin',details:{message:failed?.errors?.[0]?.message||'Royal Mail did not create this order',sheet_count:p.sheetCount,weight_grams:p.weightInGrams,postage_choice:p.choice,service_code:p.code,bulk:true}});
    }
  }
