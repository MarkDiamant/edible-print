        ...
        {!isCollection&&<section style={{...card,padding:0,overflow:'hidden'}}>
          ...
          <div style={{padding:20,display:'grid',gap:16}}>
            ...
            {royalMail?<>
              ...
              <form action={`/api/admin/orders/${id}/royal-mail`} method="post" style={{display:'flex',justifyContent:'flex-end',alignItems:'end',gap:12,flexWrap:'wrap'}}><input type="hidden" name="postage_service" value={defaultPostage}/><label style={{display:'grid',gap:5,fontSize:13,fontWeight:700}}>Package sheets<select name="sheet_count" defaultValue={String(sheetCount)} style={{minWidth:150,padding:'9px 10px',border:'1px solid #ccc',borderRadius:8,background:'#fff'}}>{Array.from({length:23},(_,i)=>i+1).map(n=><option key={n} value={n}>{n} {n===1?'sheet':'sheets'} · {83+(n-1)*30}g</option>)}</select></label><button className="btn" type="submit" style={{minWidth:220}}>Continue to Royal Mail</button></form>
            </>}
