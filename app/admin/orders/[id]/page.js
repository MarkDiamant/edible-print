...
export default async function OrderDetail({params}){
  ...
  return <main style={{maxWidth:1000,margin:'40px auto',padding:24}}><p><a href="/admin">← Orders</a></p><h1>Order #{order.order_number}</h1><p><strong>{order.first_name} {order.last_name}</strong><br/>{order.email}<br/>{order.phone||''}</p><pre style={{whiteSpace:'pre-wrap',background:'#f7f7f7',padding:16}}>{JSON.stringify(order.shipping_address,null,2)}</pre><p><strong>{order.shipping_method}</strong> · £{(order.total_pence/100).toFixed(2)} · {order.payment_status} · {order.fulfilment_status}</p><h2>Items & artwork</h2>{(items||[]).map(item=><section key={item.id} style={{borderTop:'1px solid #ddd',padding:'18px 0'}}><h3>{item.product_title}</h3><p>{item.variant_label} · Qty {item.quantity}</p>{signed.filter(a=>a.order_item_id===item.id).map(a=><p key={a.id}><a href={a.url||'#'}>{a.original_filename}</a> <small>({Math.round(a.size_bytes/1024)} KB · link valid 5 minutes)</small></p>)}</section>)}</main>;
}
