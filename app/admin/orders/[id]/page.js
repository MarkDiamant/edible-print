...
export default async function OrderDetail({params}){
  ...
  return <main style={{maxWidth:1000,margin:'40px auto',padding:24}}><p><a href="/admin">← Orders</a></p><h1>Order #{order.order_number}</h1><p><strong>{order.first_name} {order.last_name}</strong><br/>{order.email}<br/>{order.phone||''}</p><div style={{background:'#f7f7f7',padding:16,borderRadius:10,margin:'14px 0'}}><strong>Delivery address</strong><div style={{marginTop:6,lineHeight:1.5}}>{addressLines.map((line,i)=><div key={i}>{line}</div>)}</div></div><p><strong>{order.shipping_method}</strong> · £{(order.total_pence/100).toFixed(2)} · {order.payment_status} · <strong>{order.fulfilment_status}</strong></p><div style={{display:'flex',gap:10,flexWrap:'wrap',margin:'20px 0'}}>{['processing','ready','fulfilled'].map(action=><form key={action} action={`/api/admin/orders/${id}/status`} method="post"><input type="hidden" name="action" value={action}/><button className="btn" type="submit">{action==='processing'?'Mark processing':action==='ready'?'Mark ready':'Mark fulfilled'}</button></form>)}</div><div style={{display:'flex',gap:10,flexWrap:'wrap',margin:'18px 0'}}>{signed.length>0&&<a className="btn" href={signed[0].url||'#'}>Download first artwork</a>}<button className="btn" type="button" onClick={undefined} style={{display:'none'}}>Print</button></div><h2>Items & artwork</h2>{(items||[]).map(item=><section key={item.id} style={{borderTop:'1px solid #ddd',padding:'18px 0'}}><h3>{item.product_title}</h3><p>{item.variant_label} · Qty {item.quantity}</p>{signed.filter(a=>a.order_item_id===item.id).map(a=><p key={a.id}><a href={a.url||'#'}>{a.original_filename}</a> <small>({Math.round(a.size_bytes/1024)} KB · link valid 5 minutes)</small></p>)}</section>)}</main>;
}
