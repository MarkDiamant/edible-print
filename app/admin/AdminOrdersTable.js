  ...
  return <div className="admin-orders-modern">
    ...
    <div className="admin-table-wrap modern-wrap"><table className="admin-table modern-table"><thead><tr><th className="check-col"><input aria-label="Select all eligible postage" type="checkbox" checked={allSelected} onChange={toggleAll}/></th><th>Order</th><th>Date</th><th>Customer</th><th>Total</th><th>Fulfilment</th><th>Delivery status</th><th>Customer postage</th></tr></thead><tbody>{orders.map(o=>{
      ...
      return <tr key={o.id} className={`modern-row ${isNew?'new-order':''}`} onClick={e=>{if(e.target.closest('input,button,a,label'))return;openOrder(o.id,href)}}>
        ...
        <td><strong className="order-link">#{o.display_order_number}</strong>{isNew&&<span className="new-badge">NEW</span>}<div className="mobile-order-date">{new Date(o.created_at).toLocaleDateString('en-GB',{day:'2-digit',month:'short'})}</div></td>
        ...
        <td><span className={`admin-pill ${o.tracking_number?'fulfilled':'open'}`}>{o.shipping_method==='collection'?'—':o.tracking_number?'Tracking added':o.royal_mail_reference?'Postage prepared':'Not prepared'}</span>{o.tracking_number&&<div style={{fontSize:11,color:'#777',marginTop:4}}>{o.tracking_number} · <a href="https://www.royalmail.com/track-your-item" target="_blank" rel="noreferrer" onClick={e=>e.stopPropagation()} style={{fontWeight:700}}>Track ↗</a></div>}</td><td><span className={`delivery-chip ${o.shipping_method==='express'?'express':''}`}><strong>{postageLabel}</strong>{o.shipping_method!=='collection'&&<small> customer selected</small>}</span></td>
      </tr>})}</tbody></table></div>
    <style jsx>{`
      .admin-orders-modern{display:grid;gap:14px}.admin-bulk-card{display:flex;align-items:center;justify-content:space-between;gap:18px;padding:16px 18px;border:1px solid #e1e4df;border-radius:14px;background:linear-gradient(180deg,#fff,#f8faf7);box-shadow:0 3px 16px rgba(35,50,35,.05)}.admin-bulk-card>div:first-child{display:grid;gap:3px}.admin-bulk-card strong{font-size:16px}.admin-bulk-card span{font-size:13px;color:#687068}.admin-bulk-actions{display:flex;gap:8px;flex-wrap:wrap}.admin-primary,.admin-secondary{border-radius:9px;padding:9px 13px;font-weight:600;cursor:pointer;border:1px solid #cfd5cd}.admin-primary{background:#263126;color:white;border-color:#263126}.admin-primary:disabled{opacity:.45;cursor:not-allowed}.admin-secondary{background:white;color:#303630}.modern-wrap{border:1px solid #e2e5df;border-radius:14px;overflow:auto;background:white;box-shadow:0 7px 24px rgba(33,44,33,.05)}.modern-table{border-collapse:separate;border-spacing:0;width:100%;min-width:1040px}.modern-table th{font-size:12px;text-transform:uppercase;letter-spacing:.04em;color:#717771;background:#f7f8f6;padding:12px 14px;border-bottom:1px solid #e6e8e3;text-align:left;position:sticky;top:0}.modern-table td{padding:14px;border-bottom:1px solid #eceee9;vertical-align:middle}.modern-row{cursor:pointer;transition:background .14s ease}.modern-row:hover{background:#fafbf9}.modern-row.new-order{background:#fff7d6;box-shadow:inset 5px 0 0 #d49b00}.modern-row.new-order:hover{background:#fff1b8}.new-badge{display:inline-flex;margin-left:8px;padding:3px 7px;border-radius:999px;background:#d49b00;color:#fff;font-size:10px;font-weight:800;letter-spacing:.06em;vertical-align:2px}.modern-row:last-child td{border-bottom:0}.check-col{width:42px;text-align:center!important}.check-col input{width:17px;height:17px;accent-color:#6f8768;cursor:pointer}.admin-check-placeholder{color:#bcc1bb}.order-link{font-size:15px}.mobile-order-date{display:none;font-size:12px;color:#777}.delivery-chip{display:inline-grid;gap:1px;border:1px solid #e0e3dd;border-radius:10px;padding:6px 9px;background:#f8f9f7;font-size:13px}.delivery-chip.express{background:#fff7e6;border-color:#ead6a4}.delivery-chip small{font-size:11px;color:#777}.modern-table small{color:#777}.admin-pill{display:inline-flex;align-items:center;border-radius:999px;padding:5px 9px;font-size:12px;font-weight:600;white-space:nowrap}.admin-pill.open{background:#fff4cf;color:#6e5711}.admin-pill.fulfilled{background:#e9f5e8;color:#315f35}@media(max-width:760px){.admin-bulk-card{align-items:stretch;flex-direction:column}.admin-bulk-actions>*{flex:1}.modern-wrap{border-radius:12px}.modern-table{min-width:740px}.modern-table th,.modern-table td{padding:11px}.mobile-order-date{display:block}}
    `}</style>
  </div>;
}
