...
export default function AdminOrdersTable({orders}){
  ...
  const [services,setServices]=useState(()=>Object.fromEntries(eligible.map(o=>[o.id,o.shipping_method==='express'?'express':'standard'])));
  ...
  async function preparePostage(){
    ...
    setBusy(true);
    try{
      const response=await fetch('/api/admin/orders/bulk-royal-mail',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({orders:selected.map(id=>({id,postage_service:services[id]||'standard'})))});
  ...
  return <div className="admin-orders-modern">
    {eligible.length>0&&<div className="admin-bulk-card">
      <div><strong>Postage</strong><span>Select delivery orders, confirm Standard/Express, then send them to Click & Drop together.</span></div>
    ...
    <div className="admin-table-wrap modern-wrap"><table className="admin-table modern-table"><thead><tr><th className="check-col"><input aria-label="Select all eligible postage" type="checkbox" checked={allSelected} onChange={toggleAll}/></th><th>Order</th><th>Date</th><th>Customer</th><th>Total</th><th>Fulfilment</th><th>Postage</th></tr></thead><tbody>{orders.map(o=>{
      ...
      return <tr key={o.id} className="modern-row" onClick={e=>{if(e.target.closest('input,select,button,a,label'))return;router.push(href)}}>
        ...
        <td onClick={e=>e.stopPropagation()}>{o.shipping_method==='collection'?<span className="delivery-chip">Collection</span>:isEligible?<select className="postage-select" aria-label={`Postage for order ${o.display_order_number}`} value={services[o.id]||'standard'} onChange={e=>setServices(s=>({...s,[o.id]:e.target.value}))}><option value="standard">Standard</option><option value="express">Express</option></select>:<span className="delivery-chip">{o.shipping_method==='express'?'Express':'Standard'}</span>}</td>
      </tr>})}</tbody></table></div>
    <style jsx>{`
      .admin-orders-modern{display:grid;gap:14px}.admin-bulk-card{display:flex;align-items:center;justify-content:space-between;gap:18px;padding:16px 18px;border:1px solid #e1e4df;border-radius:14px;background:linear-gradient(180deg,#fff,#f8faf7);box-shadow:0 3px 16px rgba(35,50,35,.05)}.admin-bulk-card>div:first-child{display:grid;gap:3px}.admin-bulk-card strong{font-size:16px}.admin-bulk-card span{font-size:13px;color:#687068}.admin-bulk-actions{display:flex;gap:8px;flex-wrap:wrap}.admin-primary,.admin-secondary{border-radius:9px;padding:9px 13px;font-weight:600;cursor:pointer;border:1px solid #cfd5cd}.admin-primary{background:#263126;color:white;border-color:#263126}.admin-primary:disabled{opacity:.45;cursor:not-allowed}.admin-secondary{background:white;color:#303630}.modern-wrap{border:1px solid #e2e5df;border-radius:14px;overflow:auto;background:white;box-shadow:0 7px 24px rgba(33,44,33,.05)}.modern-table{border-collapse:separate;border-spacing:0;width:100%;min-width:880px}.modern-table th{font-size:12px;text-transform:uppercase;letter-spacing:.04em;color:#717771;background:#f7f8f6;padding:12px 14px;border-bottom:1px solid #e6e8e3;text-align:left;position:sticky;top:0}.modern-table td{padding:14px;border-bottom:1px solid #eceee9;vertical-align:middle}.modern-row{cursor:pointer;transition:background .14s ease,transform .14s ease}.modern-row:hover{background:#fafbf9}.modern-row:last-child td{border-bottom:0}.check-col{width:42px;text-align:center!important}.check-col input{width:17px;height:17px;accent-color:#6f8768;cursor:pointer}.admin-check-placeholder{color:#bcc1bb}.order-link{font-size:15px}.mobile-order-date{display:none;font-size:12px;color:#777}.postage-select{min-width:112px;border:1px solid #d6dad3;border-radius:8px;padding:8px 28px 8px 10px;background:#fff}.delivery-chip{display:inline-flex;border:1px solid #e0e3dd;border-radius:999px;padding:5px 9px;background:#f8f9f7;font-size:13px}.modern-table small{color:#777}.admin-pill{display:inline-flex;align-items:center;border-radius:999px;padding:5px 9px;font-size:12px;font-weight:600;white-space:nowrap}.admin-pill.open{background:#fff4cf;color:#6e5711}.admin-pill.fulfilled{background:#e9f5e8;color:#315f35}@media(max-width:760px){.admin-bulk-card{align-items:stretch;flex-direction:column}.admin-bulk-actions>*{flex:1}.modern-wrap{border-radius:12px}.modern-table{min-width:740px}.modern-table th,.modern-table td{padding:11px}.mobile-order-date{display:block}}
    `}</style>
  </div>;
}
