...
export default function Account(){
 const [user,setUser]=useState(null),[orders,setOrders]=useState([]),[loading,setLoading]=useState(true);
 ...
 async function signIn(e){e.preventDefault();const email=new FormData(e.currentTarget).get('email');const {error}=await supabase.auth.signInWithOtp({email,options:{emailRedirectTo:location.origin+'/account'}});alert(error?error.message:'Check your email for your secure sign-in link.')}
 if(loading)return <><SiteHeader/><main className="account-page"><p>Loading…</p></main><SiteFooter/></>;
 if(!user)return <><SiteHeader/><main className="account-login"><h1>Your account</h1><p>Sign in to see your orders and buy again.</p><form onSubmit={signIn}><input name="email" type="email" placeholder="Email address" required/><button className="btn">Email me a sign-in link</button></form></main><SiteFooter/></>;
