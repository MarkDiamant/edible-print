...
async function getOrder(orderId){
  ...
  const {data:rmEvents,error:rmError}=await db.from('order_events').select('event_type,details,created_at').eq('order_id',orderId).in('event_type',['royal_mail_order_created','royal_mail_order_deleted']).order('created_at',{ascending:false}).limit(50);
  if(rmError)throw rmError;
  const latestRm=rmEvents?.[0]?.event_type==='royal_mail_order_created'?rmEvents[0]:null;
