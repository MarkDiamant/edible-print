...
function shippingLabel(order){
  if(order.shipping_method==='collection')return 'Collection';
  if(order.shipping_method==='express')return 'Express delivery';
...
async function getOrder(orderId){
  ...
  const {data:rmEvents,error:rmError}=await db.from('order_events').select('event_type,details,created_at').eq('order_id',orderId).in('event_type',['royal_mail_order_created','royal_mail_tracking_updated','royal_mail_order_deleted']).order('created_at',{ascending:false}).limit(50);
  if(rmError)throw rmError;
  const latestRm=['royal_mail_order_created','royal_mail_tracking_updated'].includes(rmEvents?.[0]?.event_type)?rmEvents[0]:null;
  return {...order,items:items||[],artwork_count:artworkCount||0,royal_mail:latestRm?.details||null};
}
...
function contentFor(type,order){
  ...
  if(type==='dispatched')return {
    ...
    cta:order.royal_mail?.tracking_number?'Track your order':'View your account',href:order.royal_mail?.tracking_number?trackingHref(order):`${SITE}/account`
  };
  if(type==='collected')return {
    subject:`Order ${number} collected - Edible Print`,
    heading:'Thanks for collecting your order',
    intro:'Your Edible Print order has been collected. We hope you are delighted with your edible prints.',
    cta:'View your account',href:`${SITE}/account`
  };
...
function trackingHref(order){
  const tracking=String(order?.royal_mail?.tracking_number||'').trim();
  return tracking?`https://www.royalmail.com/track-your-item#/tracking-results/${encodeURIComponent(tracking)}`:'https://www.royalmail.com/track-your-item';
}
...
function htmlFor(type,order){
  ...
  const number=formatOrderNumber(order.order_number||order.id);
  const tracking=type==='dispatched'?String(order.royal_mail?.tracking_number||'').trim():'';
  const detailBlock=type==='feedback'?'':`
    <table width="100%" cellpadding="0" cellspacing="0" border="0" style="margin-top:24px;">
      <tr><td style="font-family:Arial,Helvetica,sans-serif;font-size:13px;line-height:18px;color:#667066;padding-bottom:8px;">ORDER ${esc(number)}</td></tr>
      ...
      <tr><td style="padding-top:8px;font-family:Arial,Helvetica,sans-serif;font-size:14px;line-height:20px;color:#454945;">${esc(shippingLabel(order))}</td><td align="right" style="padding-top:8px;font-family:Arial,Helvetica,sans-serif;font-size:14px;line-height:20px;color:#454945;">${Number(order.shipping_pence||0)===0?'Free':money(order.shipping_pence)}</td></tr>
      ...
      ${tracking?`<tr><td colspan="2" style="padding-top:18px;font-family:Arial,Helvetica,sans-serif;font-size:14px;line-height:20px;color:#454945;"><strong>Royal Mail tracking</strong><br><a href="${trackingHref(order)}" style="color:#667d60;text-decoration:underline;">${esc(tracking)}</a></td></tr>`:''}
...
function textFor(type,order){
  ...
  if(type!=='feedback'){
    lines.push(`Order: ${formatOrderNumber(order.order_number||order.id)}`);
    ...
    lines.push(`${shippingLabel(order)}: ${Number(order.shipping_pence||0)===0?'Free':money(order.shipping_pence)}`);
    lines.push(`Total: ${money(order.total_pence)}`);
    if(type==='dispatched'&&order.royal_mail?.tracking_number)lines.push(`Royal Mail tracking: ${order.royal_mail.tracking_number}`,`Track your order: ${trackingHref(order)}`);
...
export async function sendNewOrderAdminEmail(orderId){
  if(!process.env.RESEND_API_KEY)throw new Error('RESEND_API_KEY is not configured');
  const db=getSupabaseAdmin();
  const eventType='email_admin_new_order';
  const {data:already}=await db.from('order_events').select('id').eq('order_id',orderId).eq('event_type',eventType).maybeSingle();
  if(already)return {skipped:true};
  const order=await getOrder(orderId);
  const number=formatOrderNumber(order.order_number||order.id);
  const customer=`${order.first_name||''} ${order.last_name||''}`.trim()||order.email||'Customer';
  const address=addressText(order.shipping_address||{});
  const adminHref=`${SITE}/admin/orders/${order.id}`;
  const rows=(order.items||[]).map(item=>`<tr><td style="padding:10px 0;border-bottom:1px solid #ece9e2;font-family:Arial,Helvetica,sans-serif;font-size:14px;color:#2f312f;"><strong>${esc(item.product_title)}</strong><br><span style="color:#666b66;">${esc(item.variant_label||'')} &nbsp; Qty ${esc(item.quantity)}</span></td><td align="right" style="padding:10px 0;border-bottom:1px solid #ece9e2;font-family:Arial,Helvetica,sans-serif;font-size:14px;color:#2f312f;">${money((item.unit_price_pence||0)*(item.quantity||0))}</td></tr>`).join('');
  const html=`<!DOCTYPE html><html><body style="margin:0;background:#f6f3ed;"><table width="100%" cellpadding="0" cellspacing="0"><tr><td align="center" style="padding:28px 14px;"><table width="100%" cellpadding="0" cellspacing="0" style="max-width:600px;"><tr><td align="center" style="padding-bottom:18px;"><img src="${LOGO}" width="230" alt="Edible Print" style="display:block;width:230px;height:auto;"></td></tr><tr><td style="background:#fff;padding:34px 32px;border-radius:14px;font-family:Arial,Helvetica,sans-serif;color:#2f312f;"><h1 style="margin:0 0 8px;font-size:27px;">New order received</h1><p style="margin:0 0 22px;color:#555b55;">Order <strong>${esc(number)}</strong> from ${esc(customer)}</p><table width="100%" cellpadding="0" cellspacing="0">${rows}<tr><td style="padding-top:14px;font-size:14px;">${esc(shippingLabel(order))}</td><td align="right" style="padding-top:14px;font-size:14px;">${Number(order.shipping_pence||0)===0?'Free':money(order.shipping_pence)}</td></tr><tr><td style="padding-top:10px;font-size:16px;"><strong>Total</strong></td><td align="right" style="padding-top:10px;font-size:18px;"><strong>${money(order.total_pence)}</strong></td></tr></table><p style="margin:22px 0 0;font-size:14px;line-height:21px;color:#555b55;"><strong>Customer:</strong> ${esc(customer)}<br><strong>Email:</strong> ${esc(order.email||'')} ${order.phone?`<br><strong>Phone:</strong> ${esc(order.phone)}`:''}${address?`<br><strong>Address:</strong> ${esc(address)}`:''}</p><table cellpadding="0" cellspacing="0" style="margin-top:26px;"><tr><td style="background:#819b78;border-radius:8px;"><a href="${adminHref}" style="display:inline-block;padding:13px 22px;color:#fff;text-decoration:none;font-weight:bold;font-size:15px;">View order</a></td></tr></table></td></tr></table></td></tr></table></body></html>`;
  const textBody=[`New order received - ${number}`,`Customer: ${customer}`,`Email: ${order.email||''}`,order.phone?`Phone: ${order.phone}`:null,...(order.items||[]).map(item=>`${item.product_title} - ${item.variant_label||''} - Qty ${item.quantity}`),`${shippingLabel(order)}: ${Number(order.shipping_pence||0)===0?'Free':money(order.shipping_pence)}`,`Total: ${money(order.total_pence)}`,address?`Address: ${address}`:null,`View order: ${adminHref}`].filter(Boolean).join('\n');
  const response=await fetch('https://api.resend.com/emails',{method:'POST',headers:{Authorization:`Bearer ${process.env.RESEND_API_KEY}`,'Content-Type':'application/json'},body:JSON.stringify({from:FROM,to:['hello@edibleprint.uk'],subject:`New order ${number} - ${customer} - ${money(order.total_pence)}`,html,text:textBody})});
  const result=await response.json().catch(()=>({}));
  if(!response.ok)throw new Error(result.message||`Resend returned ${response.status}`);
  await db.from('order_events').insert({order_id:orderId,event_type:eventType,actor:'system',details:{resend_id:result.id||null,to:'hello@edibleprint.uk'}});
  return result;
}
