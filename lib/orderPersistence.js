...
export async function createOrder({db,cartId,items,delivery,email,phone,fullName,address,paymentIntentId,checkoutSessionId,paidAmount,actor,customerMessage=''}){
 ...
 const rows=items.map(x=>({order_id:order.id,source_draft_item_id:x.id,product_slug:x.product_slug,product_title:x.product_title,variant_label:x.variant_label,variant_index:x.variant_index,unit_price_pence:x.unit_price_pence,quantity:x.quantity,max_designs:x.max_designs,design_service:Boolean(x.design_service),print_fit:x.print_fit||null}));
